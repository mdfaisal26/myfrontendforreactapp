...
function HomeGuest() {
  ...
  useEffect(() => {
    if (state.username.checkCount) {
      ...
      async function fetchResults() {
        try {
          const response = await Axios.post(`/doesUsernameExist`, { username: state.username.value }, { signal: ourRequest.signal });
          dispatch({ type: "usernameUniqueResults", value: response.data });
        } catch (e) {
          console.log("There was a problem in getting username");
        }
      }
      fetchResults();
      return () => {
  ...
  useEffect(() => {
    if (state.email.checkCount) {
      ...
      async function fetchResults() {
        try {
          const response = await Axios.post(`/doesEmailExist`, { email: state.email.value }, { signal: ourRequest.signal });
          dispatch({ type: "emailUniqueResults", value: response.data });
        } catch (e) {
          console.log("There was a problem in getting email");
        }
      }
      fetchResults();
      return () => {
  ...
  useEffect(() => {
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          const response = await Axios.post(`/register`, { username: state.username.value, email: state.email.value, password: state.password.value }, { signal: ourRequest.signal });
