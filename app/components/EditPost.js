...
function EditPost() {
  ...
  function submitHandler(e) {
    e.preventDefault();
    dispatch({ type: "titleRules", value: state.title.message });
    dispatch({ type: "bodyRules", value: state.body.message });
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        const response = await Axios.get(`/post/${state.id}`, { signal: ourRequest.signal });
        if (response.data) {
  ...
  useEffect(() => {
    if (state.sendCount) {
      dispatch({ type: "saveRequestStrted" });
      const ourRequest = new AbortController();
      async function fetchPost() {
        try {
          const response = await Axios.post(`/post/${state.id}/edit`, { title: state.title.value, body: state.body.value, token: appState.user.token }, { signal: ourRequest.signal });
